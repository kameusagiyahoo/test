...
function saveDraft(){
  session.savePlayers(draftPlayers);draftPlayers=[...session.players];updateBadge();toast('メンバーを保存しました');
}
...
function renderHome(){
  disposeActiveGame();draftPlayers=[...session.players];updateBadge(`${session.players.length}人 · ${listGames().length} games`);
  app.innerHTML=`<section class="hero"><h1>集まったら、<br>すぐ遊ぶ。</h1><p>2〜8人。スマホ1台。6ラウンドの総合戦にも対応。</p></section>
  ...
  <section class="mode-grid"><button class="game-card" id="partyMode" style="background:#fff0bd"><div class="chip">6 ROUNDS</div><h3>🏆 Party Mode</h3><p>3ゲームを各2回。各ラウンドの順位を3/2/1 Party Pointへ変換して総合優勝を決める。</p></button><div class="card" style="padding:18px"><div class="chip">SINGLE</div><h3>1ゲームだけ遊ぶ</h3><p class="muted">好きなゲームを選び、先に5点取った人が勝利。</p></div></section>
  <div class="section-head"><h2>ゲーム</h2><span class="muted">モジュール式</span></div>
  <section class="games">${listGames().map(g=>`<button class="game-card" data-game="${g.id}"><div style="font-size:32px">${g.emoji}</div><h3>${g.title}</h3><p>${g.description}</p><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:12px">${g.tags.map(t=>`<span class="chip">${t}</span>`).join('')}</div></button>`).join('')}</section><div class="footer">Party Pocket v3 · ready for remote transport</div>`;
  renderPlayers();
