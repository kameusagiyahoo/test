...
function nextPlayer(ctx,state,life){
  ...
  const puzzle=makePatternPuzzle(),name=ctx.session.players[state.player];
  ctx.renderScorebar(state.player);
  ctx.root.innerHTML=`<div class="pass-card"><div class="eyebrow">PATTERN CODE</div><div class="prompt">${ctx.esc(name)}さんへ</div><div class="sub">数列の規則を見抜いて次の数を選びます。</div><button class="btn primary" id="patternReady">問題を見る</button></div>`;
  ctx.root.querySelector('#patternReady').onclick=()=>play(ctx,state,life,puzzle);
}

function play(ctx,state,life,puzzle){
  ctx.root.innerHTML=`<div class="eyebrow">NEXT NUMBER</div><div class="pattern-sequence">${puzzle.seq.join(' · ')} · ?</div><div class="choice-grid">${puzzle.choices.map(v=>`<button class="choice" data-pattern="${v}">${v}</button>`).join('')}</div><div class="rules">規則は一定加算・一定減算・交互変化・差が増える、などから出題されます。</div>`;
