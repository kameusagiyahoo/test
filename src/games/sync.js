...
export const syncGame={
  ...
  mount(ctx){
    const life={destroyed:false,holdTimer:null};
    startRound(ctx,life);
    return()=>{life.destroyed=true;clearTimeout(life.holdTimer)};
  }
};

function startRound(ctx,life){if(life.destroyed)return;const free=Math.random()<0.45;const round=free?{type:'free',q:pick(freePrompts)}:(()=>{const [q,a]=pick(choiceRounds);return{type:'choice',q,a}})();pass(ctx,{round,player:0,answers:[]},life)}
...
function showQuestion(ctx,state,life){
  if(life.destroyed)return;const r=state.round;
  if(r.type==='choice'){
    ...
    ctx.root.querySelectorAll('[data-answer]').forEach(b=>b.onclick=()=>submit(ctx,state,{key:`c${b.dataset.answer}`,label:r.a[+b.dataset.answer]},life));
  }else{
...
function submit(ctx,state,answer,life){if(life.destroyed)return;state.answers.push(answer);state.player++;if(state.player<ctx.session.players.length)pass(ctx,state,life);else reveal(ctx,state,life)}
function reveal(ctx,state,life){
  if(life.destroyed)return;const counts={};state.answers.forEach(a=>counts[a.key]=(counts[a.key]||0)+1);state.answers.forEach((a,i)=>{if(counts[a.key]>=2)ctx.session.addScore(i,counts[a.key]-1)});ctx.renderScorebar();
